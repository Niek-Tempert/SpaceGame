...
inline void Player::update(const Engine* engine) {
	glm::vec2 delta = { engine->input->get_cursor_delta().x, engine->input->get_cursor_delta().y };
	...
	if (engine->input->is_key_down(GLFW_KEY_SPACE)) {
		dir.y += 1;
	}

	if (engine->input->is_key_down(GLFW_KEY_LEFT_CONTROL)) {
		dir.y -= 1;
	}
	...
	if (glm::length(dir) > 0) {
		...
		f32 speed;
		if (engine->input->is_key_down(GLFW_KEY_LEFT_SHIFT)) {
			speed = RUN_SPEED;
		} else {
			speed = WALK_SPEED;
		}
			
		position += rotated_dir * (f32)engine->delta_time * speed;
	}
}
