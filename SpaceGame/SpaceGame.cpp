...
inline void render(const Program *program) {
	...
	glm::mat4 proj;
	if (!glm::isnan(ratio) && !glm::isinf(ratio)) { // Prevents a crash when the window is minimized
...
inline void on_update(const Program *program) {
	...
	if (result.hit) {
		...
		if (program->focussed && program->input->is_mouse_press(GLFW_MOUSE_BUTTON_1) && result.hit) {
			hit_voxel->set(result.id, { NULL, NULL });
			hit_voxel->update(result.id);
		}

		if (program->focussed && program->input->is_mouse_press(GLFW_MOUSE_BUTTON_2) && result.hit) {
...
inline void start(Program *&program) {
	...
	glfwSetWindowMonitor(window, NULL, mode->width / 4, mode->height / 4, mode->width / 2, mode->height / 2, GLFW_DONT_CARE);
	...
	while (!glfwWindowShouldClose(program->window)) {
		...
		if (program->input->is_key_press(GLFW_KEY_F11)) {
			if (program->fullscreen) {
				...
				glfwSetWindowMonitor(program->window, NULL, mode->width / 4, mode->height / 4, mode->width / 2, mode->height / 2, GLFW_DONT_CARE);
				program->fullscreen = false;
			} else {
				GLFWmonitor *monitor = glfwGetPrimaryMonitor();
				const GLFWvidmode *mode = glfwGetVideoMode(monitor);

				glfwSetWindowMonitor(program->window, monitor, 0, 0, mode->width, mode->height, GLFW_DONT_CARE);
				program->fullscreen = true;
			}
		}

		if (program->input->is_key_press(GLFW_KEY_ESCAPE)) {
			if (program->focussed) {
				glfwSetInputMode(program->window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
				program->focussed = false;
			}
		}
