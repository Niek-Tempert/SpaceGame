...
static constexpr FORCEINLINE NODISCARD bool is_even(const int_t a) { \
	return (a % (int_t)2) == (int_t)0; \
} \
static constexpr FORCEINLINE NODISCARD bool is_odd(const int_t a) { \
	return (a % (int_t)2) != (int_t)0; \
} \
/* Floored modulo: Continuous positive */ \
static FORCEINLINE NODISCARD int_t floor_mod(const int_t a, const int_t b) { \
	const int_t r = a % b; \
	return (r < (int_t)0) ? r + b : r; \
}

#define DEFINE_FLOAT_INT_MATH(float_t, int_t) \
static constexpr FORCEINLINE NODISCARD int_t floor_to_##int_t(const float_t a) { \
	int_t i = (int_t)a; \
	i -= (float_t)i > a; \
	return i; \
}
...
static FORCEINLINE NODISCARD float_t floor_mod(const float_t a, const float_t b) { \
	const float_t r = fmod(a, b); \
	return (r < (float_t)0) ? r + b : r; \
} \
static constexpr FORCEINLINE NODISCARD float_t invlerp(const float_t a, const float_t b, const float_t v) { \
	return (v - a) / (b - a); \
} \
static constexpr FORCEINLINE NODISCARD float_t remap(const float_t v, const float_t imin, const float_t imax, const float_t omin, const float_t omax) { \
	return omin + (v - imin) * (omax - omin) / (imax - imin); \
} \
static FORCEINLINE NODISCARD float_t snap(const float_t v, const float_t i) { \
	return floor(v / i) * i; \
} \
static constexpr FORCEINLINE NODISCARD float_t wrap(const float_t v, const float_t a, const float_t b) { \
	if (a - b == (float_t)0) { \
		return b; \
	} \
...
static constexpr FORCEINLINE NODISCARD float_t smin(const float_t v, const float_t l, const float_t s) { \
	if (s == (float_t)0) { \
		return min(v, l); \
	} \
...
static constexpr FORCEINLINE NODISCARD float_t smax(float_t v, float_t l, float_t s) { \
	return -smin(-v, -l, s); \
}

#define DEFINE_NUMBER_MATH(number_t) \
/* TODO: See if native min/max functions are faster */ \
static constexpr FORCEINLINE NODISCARD number_t min(const number_t a, const number_t b) { \
	return a < b ? a : b; \
} \
static constexpr FORCEINLINE NODISCARD number_t max(const number_t a, const number_t b) { \
	return a > b ? a : b; \
} \
static constexpr FORCEINLINE NODISCARD number_t clamp(const number_t x, const number_t a, const number_t b) { \
	return max(a, min(b, x)); \
} \
/* Return 0 if the divisor is 0 */ \
static constexpr FORCEINLINE NODISCARD number_t save_divide(const number_t a, const number_t b) { \
	return (b != (number_t)0) ? a / b : (number_t)0; \
}

#define DEFINE_SIGNED_MATH(signed_t) \
static constexpr FORCEINLINE NODISCARD signed_t abs(const signed_t a) { \
	return max((signed_t)-a, a); \
}

#define DEFINE_UNSIGNED_MATH(unsigned_t) \
static constexpr FORCEINLINE NODISCARD unsigned_t abs(const unsigned_t a) { \
	return a; \
}
